...
camera = Camera3D(glm.vec3(0.0, 0.0, 10.0))
...
def init():
    ...
    print(grid_vertices.shape[0] / 5)

    global shader_program
    shader_program = ShaderProgram("shaders/shader.vs", "shaders/shader.fg")
    shader_program.init()
...
def drawFunc():
    ...
    for position in grid_position:
        ...
        m = glm.mat4(1.0)
        m = glm.translate(m, position[2])
        # m = glm.rotate(m, glm.radians(glutGet(GLUT_ELAPSED_TIME) / 10), glm.vec3(1.0, 1.0, -1.0))
        m = glm.rotate(m, position[1][0], position[1][1])
    ...
    global delta_time
    delta_time = (last_frame - current_frame)  # 毫秒
